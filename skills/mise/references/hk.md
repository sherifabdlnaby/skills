...
- **Drive one `check` task off this**, not two: stay on `hk check` and forward an opt-in `--fix` — no branching on subcommand. That `check` task is the repo's standard lint contract (same command CI and the pre-commit hook run); see [`reference-setup-and-patterns.md`](reference-setup-and-patterns.md#check-lint).
