...
├── mise.lock                  # generated by `mise lock`, committed with anything it writes beside it
├── .config/
│   ├── hk.pkl                 # git hooks and linters
│   └── mise/
│       ├── .gitignore         # ignores the local setup stamp
│       └── tasks/             # file tasks, when a task outgrows TOML
├── .github/workflows/
│   ├── check.yml              # report-only lint on PRs
│   └── check.autofix.yml      # opt-in variant that pushes fixes
├── AGENTS.md                  # how agents run and extend the setup
└── README.md                  # install mise, trust, setup, everyday commands
```
...
Order lives in SKILL.md "Always applies".
...
Prerequisites mise can't install (Docker running, VPN reachable, supported OS) are
`[doctor.checks.<name>]` entries: a `run` probe, a `description`, and a `hint` that says exactly
how to fix it. Setup runs `mise doctor project` before anything slow, so a machine that can't
finish fails in seconds. **Check-and-instruct only** — anything a script can fix is a regular
setup step. With no checks declared the command prints a notice and passes, so setup keeps the
...
itself on the next `mise run`, and `deps` self-heals from its cache, so adding or bumping a tool or a dependency leaves the stamp alone.
