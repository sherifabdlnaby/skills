...
- **Publish with generated notes.** `gh release create <tag> --generate-notes`. release-drafter's
  ...
  autolabeler taxonomy to the repo's real change types.
...
- **Release notes carry a Verify + Install footer.** Generated notes (`--generate-notes`) say *what
  changed*; the release job appends a fixed footer saying how to *trust and use* the artifact. why: an
  unverifiable signature protects no one, and a consumer who can't find the installation step won't run it.
